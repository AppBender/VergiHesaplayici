    ...
    for idx, row in df.iterrows():
        ...
        if row.iloc[0] == 'Statement':
            if row.iloc[1] == 'Header':
                db.log_info(f"Row {idx}: Statement Header")
            elif row.iloc[1] == 'Data':
                db.log_info(f"Row {idx}: Statement Data - {row.iloc[2]}: {row.iloc[3]}")

        elif row.iloc[0] == 'Account Information':
            db.log_info(f"Row {idx}: Account Info - {row.iloc[2]}: {row.iloc[3]}")

        elif 'Trades' in str(row.values):
            db.log_info(f"Row {idx}: Trades section start")

        elif pd.isna(row.iloc[0]):
            db.log_info(f"Row {idx}: Empty row")

        else:
            db.log_info(f"Row {idx}: Other type - {row.iloc[0]}")

        db.log_info(f"Satır {idx+1}: {row_type}")
        db.log_info(f"İçerik: {', '.join(str(x) for x in row.values if pd.notna(x))}")
        db.log_info("-" * 50)
